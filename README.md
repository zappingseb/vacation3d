...
vacations/<id>.json         THE config of one vacation: title, POIs, break names, camera, colours
clean_tool/clean_gpx.py     GPX + json -> data/<id>.js (cleaning, stop detection, day split)
data/<id>.js                the committed data of one vacation: tracks, breaks, interest_breaks, pois, config
vacations/<id>.php          plugin main file of one vacation: WordPress header + the id, nothing else
...
plugins/vacation-3d-map-<id>/   build output of build_plugin.sh, gitignored
...
```bash
# 1. drop the GPX files of the trip into raw/<id>/ and run the cleaner once to see days and breaks
...
# 2. write vacations/<id>.json: title, pois, break names ("2:54": "Geisleralm"), camera, colours
#    (copy vacations/vilnoess.json). Run the cleaner again -> data/<id>.js has everything.
python3 clean_tool/clean_gpx.py --id vilnoess
# 3. assemble the plugin folder + zip
./build_plugin.sh vilnoess
```

With Claude Code, say "new vacation" — the `new-vacation` skill walks through GPX, day split,
break naming, POIs and camera as an interview.
...
Why both `vacations/<id>.json` and `data/<id>.js` are committed: the json is what you edit, the
js is what the browser loads, and it also contains the cleaned tracks, whose GPX sources are
deliberately not in git. Everything else (plugin folder, zip) is build output.
...
window.VACATION3D_DATA["vilnoess"] = {
  ...
  interest_breaks: FeatureCollection<Point>  pauses named in the json      -> orange dot + label
...
Everything the map shows comes from this one file. The block and the demo only pass presentation
overrides (`overview.zoom` from the block's zoom setting, `hash` for the demo). Day split: one GPX
file = one day by default; `"days": "date"` in the json pools all points and splits by calendar date.
...
Run `./build_plugin.sh <id>` first (the plugin folder is not in git). Then either upload
`dist/vacation-3d-map-<id>.zip` in *wp-admin → Plugins → Upload*, or push the folder over FTP
with the music_blog tooling (health check + automatic rollback):
...
2. Copy `vacations/vilnoess.json` to `vacations/<newid>.json` (title, POIs, break names, camera)
   and `vacations/vilnoess.php` to `vacations/<newid>.php` (plugin name + the id), rerun the cleaner.
