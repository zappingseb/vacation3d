...
(function () {
  ...
  function mount(container, userConfig, data) {
    ...
    let state = 'idle', elapsed = 0, lastFrame = null, raf = 0, follow = true, cursorKm = null, speed = 1;
    ...
    function render() {
      ...
      if (follow) {
        // one jumpTo per frame with lerped center/zoom/pitch; an easeTo in parallel would be cancelled by it
        const c = map.getCenter();
        map.jumpTo({ center: [lerp(c.lng, lon, 0.06), lerp(c.lat, lat, 0.06)],
                     zoom: lerp(map.getZoom(), cfg.follow.zoom, 0.04), pitch: lerp(map.getPitch(), cfg.follow.pitch, 0.04) });
      }
      if (!profileEl.hidden) drawProfile();
    }
    function play() {
      if (state === 'playing') return;
      if (state === 'idle') {
        elapsed = 0; follow = true;
    ...
    function stop() {
      ...
      setState('idle'); cancelAnimationFrame(raf); hiker.remove(); cursorKm = null; drawProfile();
