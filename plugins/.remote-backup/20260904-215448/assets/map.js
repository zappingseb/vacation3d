 ...
 *              breaks[ {day, minutes, label} ]  (names a detected break, e.g. a hut you had lunch at),
 *              autoplay?, hash? }
...
(function () {
  ...
  function mount(container, userConfig, data) {
    ...
    const TRACKS = data.tracks, BREAKS = data.breaks || { type: 'FeatureCollection', features: [] };
    ...
    // named breaks: config.breaks = [{day, minutes, label}] matches a break by day and duration
    // and gives it a label like the huts (smaller), plus the name in the popup
    for (const nb of cfg.breaks || []) {
      const f = BREAKS.features.find(b => b.properties.day === nb.day && b.properties.minutes === nb.minutes);
      if (!f) continue;
      f.properties.name = nb.label;
      const el = h('div', 'v3d-poi v3d-poi--break', `<div class="dot"></div><div class="lbl">${nb.label}</div>`);
    ...
    map.on('mouseenter', 'breaks', e => {
      ...
      popup.setLngLat(e.lngLat).setHTML(`${p.name ? '<b>' + p.name + '</b><br>' : ''}Tag ${p.day}, ${p.time} UTC<br>Pause ${p.minutes} min, ${Math.round(p.ele)} m`).addTo(map);
    });
